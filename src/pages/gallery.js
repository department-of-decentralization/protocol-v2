...
const GalleryPage = () => {
  ...
  const thumbswithSrc = images.map((image) => {
    return {
      ...image,
      src: `https://raw.githubusercontent.com/Department-of-Decentralization/pbv2-photos/main/thumbnails/${image.name}`,
    };
  });

  const imagesWithSrc = images.map((image) => {
    return {
      src: `https://raw.githubusercontent.com/Department-of-Decentralization/pbv2-photos/main/${image.name}`,
    };
  });
  ...
  return (
    <main className="my-24">
      <header className="text-center">
        <h1 className="font-bold">Protocol Berg v2</h1>
      ...
      <section>
        ...
        <p>
          ...
          helped us to document Protocol Berg's unique atmosphere and people. If you like the photos, please consider
          sending Anton a donation to <code>antontal.eth</code>
        </p>
        <div>
          <Gallery
      ...
      <p className="ml-12">
        Back to <a href="/">protocol.berlin</a>
      </p>
    </main>
  );
};
...
export const Head = () => <title>Protocol Berg v2 - Gallery</title>;
